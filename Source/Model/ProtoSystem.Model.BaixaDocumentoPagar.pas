...
type
  TfrmBaixaDocumentosPagar = class(TForm)
    Panel1: TPanel;
    Panel8: TPanel;
    Pnl_sair: TPanel;
    Shape19: TShape;
...
procedure TfrmBaixaDocumentosPagar.GeraMovimentações;
var ValorMov: double;
begin
...
case IndexCond of
  ...
  begin //caixa
    ...
    dm.FDQuery.ParamByName('OBSERVACAO').AsString := 'LANÇAMENTO GERADO PELA BAIXA DE DOCUMENTOS A PAGAR';
    dm.FDQuery.ParamByName('TIPO').AsString :='SAIDA';
    dm.FDQuery.ExecSQL;
    dm.FDQuery.Close;
  end;
...
procedure TfrmBaixaDocumentosPagar.gpTipoPagClick(Sender: TObject);
begin
  IndexCond:=gpTipoPag.ItemIndex;
end;

procedure TfrmBaixaDocumentosPagar.AtualizaStatusBaixado;
var
IDDOC: INTEGER;
begin
  ...
  dm.FDQuery.SQL.Text :='UPDATE DOC_PAGAR SET QUITADO ="S" WHERE ID= :ID';
...
procedure TfrmBaixaDocumentosPagar.btnComfirmarClick(Sender: TObject);
begin
  GeraMovimentações;
  AtualizaStatusBaixado;
end;

procedure TfrmBaixaDocumentosPagar.btn_FecharClick(Sender: TObject);
begin
  close;
end;

procedure TfrmBaixaDocumentosPagar.FormCreate(Sender: TObject);
begin
